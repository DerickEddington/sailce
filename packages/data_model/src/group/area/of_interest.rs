...
{
    /// To be included in this `AreaOfInterest`, an `Entry` must be [included](Area::includes) in
    /// this `Area`.
...
impl From<Max> for Option<u64>
{
    #[inline]
    fn from(value: Max) -> Self
    {
        match value {
            Max::Limit(limit) => Some(limit.get()),
            Max::Unlimited => None,
        }
    }
}
    ...
    P: Path,
{
    ...
    /// - `self.area` [includes](Area::includes) `entry`,
    ...
    #[must_use]
    #[inline]
    pub async fn includes<Params, Pe>(
        &self,
    ...
    where
        ...
        Pe: Path,
    {
        use crate::store::r#async::Store as _;
        ...
        same_namespace() && within_area() && within_limits.await
    }
...
/// This is the `AreaOfInterest` that includes the entirety of **all** [`Entry`]s in a Namespace
/// in a [`Store`].
///
/// (This is analogous to [`ThreeDimRange::default`](crate::group::ThreeDimRange::default), but
/// was not part of the Willow documents (as of 2024-03), but this would seem to be appropriate.)
impl<S, P> Default for AreaOfInterest<S, P>
where
    S: Eq,
    P: EmptyPath,
{
    ...
    fn default() -> Self
    {
        Self { area: Area::default(), max_count: Max::Unlimited, max_size: Max::Unlimited }
    }
}
