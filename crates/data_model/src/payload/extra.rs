...
pub trait ExtraCore: Payload
{
    ...
    ///
    /// # Errors
    /// If `payload`'s implementation of [`Payload::seek`] errors.
    #[inline]
    ...
    /// be useful for computing the [`payload_digest`](crate::Entry::payload_digest), when the
    /// range is the entire payload, i.e. `0 .. payload.len()`, at the same time as our
    /// copying, which e.g. can be useful for [`Store::put`](crate::store::async::Store::put)
    ...
    /// bytes.  Note that `callback` is not `async` (does not return a `Future`) and so must
    /// not block.
    ///
    /// # Errors
    /// - If `start` or `start + dest.len()` is outside the bounds of `payload`.
...
mod alloc
{
    ...
    pub trait Extra: ExtraCore
    {
        ...
        ///
        /// If `range.start_bound()` is `Unbounded`, the current position of `payload` is used as
        ...
        ///
        /// # Errors
        /// - If `range` is outside the bounds of `payload`.
