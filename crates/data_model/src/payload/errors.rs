...
{
    /// One of the range arguments is out-of-bounds of the length of the `payload` argument.
    OutOfBounds
    {
    ...
    Seek(SeekError),
    /// The `payload` argument's implementation of [`Payload`](crate::Payload) violated
    /// required behavior.
    BadImpl,
}
...
mod alloc
{
    ...
    {
        /// The `range` argument is out-of-bounds of the length of the `payload` argument.
        OutOfBounds
        {
            /// The position that is out-of-bounds, or `None` if overflow occurred.
            at: Option<NonZeroU64>,
