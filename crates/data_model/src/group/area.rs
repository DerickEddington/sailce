...
/// Determines whether [`Area::subspace`](struct.Area.html#structfield.subspace) denotes a single
/// Subspace or all Subspaces.
#[derive(Copy, Clone, Eq, Ord, Hash, PartialEq, PartialOrd, Debug)]
...
mod sealed
{
        ...
        P: Path,
    {
        ...
        {
            self.is_empty()
                || (other.times.includes_range(&self.times) // First, in case `other` is empty.
...
/// This is the `Area` that includes the entirety of **all** [`Entry`]s in a Namespace.
///
...
mod tests
{
    ...
    mod subspace
    {
        ...
        fn includes()
        {
            ...
            assert!(Id(0).includes(&Id(0)));
            assert!(!Id(0).includes(&Id(1)));
            assert!(!Id(1).includes(&Id(0)));
            assert!(!Id(i32::MAX).includes(&Any::<i32>));
            assert!(Any.includes(&Any::<char>));
            assert!(Any.includes(&Id('0')));
            assert!(Any.includes(&Id(char::MAX)));
        }
    }
}
