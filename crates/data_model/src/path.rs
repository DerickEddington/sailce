...
/// a byte-string.
///
/// This makes various standard (and 3rd-party) types usable as `Path`s, e.g.: `[&[u8]; N]`,
/// `&[&str]`, `Vec<String>`, etc.  This enables flexibility, for different situations, in what is
/// used to represent `Path`s, e.g.: `[b"a", b"b"]`, `&["a", "b"][..]`, `vec!["a".to_owned(),
/// "b".to_owned()]`, etc.
impl<T, C> Path for T
where
    T: ?Sized,
...
/// A [`Path`] that can represent the empty `Path`, i.e. that has no `Component`s.
///
...
/// Automatically implemented for any type that can be constructed by
/// [`FromIterator<Component>`](FromIterator).
///
